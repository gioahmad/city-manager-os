...
REPAIR_ID="issue-58-parcel-context-repair-v1"
...
SELECT
  ...
  AND EXISTS (SELECT 1 FROM spatial_reference_entities WHERE active=true)
  AND EXISTS (
  ...
  AND has_table_privilege('citymanager_app','spatial_reference_entities','SELECT')
  AND has_table_privilege('citymanager_app','spatial_reference_catalog_status','SELECT')
  AND has_function_privilege(
    'citymanager_app','gis_parcel_context(integer,double precision)','EXECUTE'
  );
SQL
)" == t ]]
}
...
database_ready(){
  partial_database_ready || return 1
  [[ "$(db_at <<'SQL'
...
if [[ "$HEAD_SHA" == "$FAILED_RELEASE_HEAD" ]]; then
  REPAIR_MODE="build"
  [[ "$ORIGIN_SHA" == "$BOOTSTRAP_HEAD" ]] || fail "origin/main moved beyond the #58 bootstrap"
  hash_is deploy/postgis/init/031_spatial_reference_catalog.sql 4147b389c4fb16df53537da1799958699c0c8dc5bc83c8c79b7d40e0157f5637 \
    || fail "failed SQL source does not match the diagnosed release"
  hash_is deploy/gis/install_spatial_reference_catalog.sh 15e72db65814aff921a1463962d519253616873750d23612a26bc242bc3dc46d \
    || fail "failed installer does not match the diagnosed release"
  hash_is dashboard/tests/test_spatial_reference_catalog.py 95a247ea74ef2ffaf889ffc6a16951916a763534704f9597e1085717b936d273 \
    || fail "failed regression test does not match the diagnosed release"
elif [[ "$(git rev-parse HEAD^)" == "$FAILED_RELEASE_HEAD" ]] \
  && [[ "$(git diff --name-only "$FAILED_RELEASE_HEAD"..HEAD | sort)" == "$REPAIR_PATHS" ]]; then
  REPAIR_MODE="resume"
  TARGET_HEAD="$HEAD_SHA"
  [[ "$ORIGIN_SHA" == "$BOOTSTRAP_HEAD" || "$ORIGIN_SHA" == "$TARGET_HEAD" ]] \
    || fail "origin/main is neither the #58 bootstrap nor this repair target"
else
  fail "local main is not the exact failed #58 release or its resumable repair"
fi
...
partial_database_ready || fail "production database does not match the safe post-refresh #58 state"
...
CURRENT_PHASE="code"
section "2. GUARDED THREE-FILE CODE REPAIR"
if [[ "$REPAIR_MODE" == build ]]; then
...
diff --git a/deploy/gis/install_spatial_reference_catalog.sh b/deploy/gis/install_spatial_reference_catalog.sh
index 139c72e..e6142ed 100755
--- a/deploy/gis/install_spatial_reference_catalog.sh
+++ b/deploy/gis/install_spatial_reference_catalog.sh
...
PATCH
  ...
  printf '%s\n' "$PATCH_CONTENT" | git apply --check
  printf '%s\n' "$PATCH_CONTENT" | git apply
  [[ "$(git diff --name-only | sort)" == "$REPAIR_PATHS" ]] || fail "repair changed an unexpected path"
  git diff --check
  ...
  hash_is dashboard/tests/test_spatial_reference_catalog.py bb52c4bbff76e1946131295c6ab73ecab4901184560c21aa288b662fe1330fa4 \
    || fail "corrected regression test checksum mismatch"
...
spec = importlib.util.spec_from_file_location("issue58_repair_static", path)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
for name in sorted(item for item in dir(module) if item.startswith("test_")):
    getattr(module, name)()
    print("REPAIR STATIC PASS", name)
PY
  git add -- $REPAIR_PATHS
  [[ "$(git diff --cached --name-only | sort)" == "$REPAIR_PATHS" ]] || fail "staged repair paths differ from manifest"
  git diff --cached --check
  git -c user.name='City Manager OS Release' -c user.email='release@localhost' \
    commit -m "Fix #58 parcel context SQL validation"
  ...
  hash_is dashboard/tests/test_spatial_reference_catalog.py bb52c4bbff76e1946131295c6ab73ecab4901184560c21aa288b662fe1330fa4 \
    || fail "resumable regression test checksum mismatch"
fi
[[ "$(git rev-parse HEAD^)" == "$FAILED_RELEASE_HEAD" ]] || fail "repair target parent mismatch"
...
if database_ready 2>/dev/null; then
  ...
  database_ready || fail "database context/topology readiness failed after the transactional repair"
fi
...
[[ "$CATALOG_AFTER" == "$CATALOG_BEFORE" ]] || fail "catalog row/link counts changed during the repair"
