...
trap 'rc=$?; log "COMMAND NAV HOTFIX FAILED with exit code ${rc}. Review the log before retrying."; exit $rc' ERR
...
grep -q 'command_nav.css?v=20260903-1' dashboard/templates/nav.html || fail "Command nav stylesheet marker missing."
grep -q 'Desktop command ribbon' dashboard/static/command_nav.css || fail "Command nav CSS missing."
grep -q 'background:linear-gradient(90deg,#0d2439' dashboard/static/command_nav.css || fail "Command ribbon skin marker missing."
...
for _ in $(seq 1 20); do
  if docker exec citymanager-dashboard python -c "import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:8000/health',timeout=5).read().decode())" >/tmp/cmos-command-nav-health.txt 2>/dev/null; then
    OK=1
    break
  fi
  sleep 2
...
cat /tmp/cmos-command-nav-health.txt

log "Verifying command ribbon on representative pages"
...
for path in ['/', '/issues', '/staff-admin', '/rules']:
    ...
    required=['desktop-nav','Operations Board','Rules Center','mobile-nav','command_nav.css?v=20260903-1']
    missing=[x for x in required if x not in body]
    if missing:
        raise SystemExit(f'{path}: missing {missing}')
    print(f'{path}: command nav OK')
PY
