...
SELECT objectid::bigint,
       addresspointid::text,
       regexp_replace(trim(concat_ws(' ',house_number,house_number_suffix,full_street_name)),'\s+',' ','g') AS fulladdr,
       CASE boroughcode::text WHEN '1' THEN 'Manhattan' WHEN '2' THEN 'Bronx'
       ...
       'NY'::text AS state,
       CASE boroughcode::text WHEN '1' THEN 'New York' WHEN '2' THEN 'Bronx'
...
WHERE geom IS NOT NULL AND boroughcode::text IN ('1','2','3','4','5')
  AND nullif(trim(house_number),'') IS NOT NULL AND nullif(trim(full_street_name),'') IS NOT NULL;
ALTER TABLE gis_nyc_addresses_build ADD PRIMARY KEY(objectid);
ALTER TABLE gis_nyc_addresses_build ADD UNIQUE(addresspointid);
...
ALTER TABLE gis_nyc_addresses RENAME CONSTRAINT gis_nyc_addresses_build_addresspointid_key TO gis_nyc_addresses_addresspointid_key;
